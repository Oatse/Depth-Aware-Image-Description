...
from services.capture_repository import CaptureRepository
...
def test_capture_rejects_missing_ground_truth(monkeypatch, tmp_path) -> None:
    ...
    assert not (tmp_path / "captures" / "records").exists()
...
def test_capture_is_persisted_with_sensor_metadata_without_running_gemma(monkeypatch, tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        count = client.get("/captures/count", params={"batch_id": "batch-001"})
        listing = client.get("/captures", params={"batch_id": "batch-001"})
    ...
    assert count.json() == {"count": 1, "batch_id": "batch-001"}
    ...
    with TestClient(app) as client:
        ...
        for _ in range(100):
            ...
            if result["status"] in {"completed", "failed"}:
                break
            time.sleep(0.01)
        stored = client.get("/captures", params={"batch_id": "batch-001"}).json()["captures"][0]
