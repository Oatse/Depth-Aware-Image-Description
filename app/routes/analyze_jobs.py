...
async def create_analysis_job(
    ...
    try:
        ...
        if capture_time_ms is not None:
            host_match_time_ms = int(time.time() * 1000)
            if camera_facing_mode == "user":
                ...
                sensor_evidence = request.app.state.sensor_bridge.snapshot(
                    host_match_time_ms,
                    analyze_route.settings.sensor_match_window_ms,
                )
            sensor_evidence["capture_id"] = capture_id
            sensor_evidence["client_capture_time_ms"] = capture_time_ms
            sensor_evidence["host_match_time_ms"] = host_match_time_ms
            sensor_evidence["camera_facing_mode"] = camera_facing_mode
